...
Public Class PJClientMain
    Dim xStationXML As New StationXML
    ...
    Private PageURL() = {"http://www.pingjockey.net/cgi-bin/pingtalk/", "http://www.chris.org/cgi-bin/jt65emeA", _
                          "http://www.chris.org/cgi-bin/jt65emeB", "http://www.chris.org/cgi-bin/jt65talk"}
    ...
    Private Sub CopyFilesIfNewInstall()
        Try
            Dim oldDataPath = DataPath.Split("\")
            ...
            '    If Not System.IO.File.Exists(OldPath & "\Colors.xml") Then
            '        System.IO.File.Copy(ApplicationPath & "\AppData\Colors.xml", DataPath & "\Colors.xml")
            '    Else
            '        System.IO.File.Copy(OldPath & "\Colors.xml", DataPath & "\Colors.xml")
            '    End If
            'End If
            copyXmlFiles(OldPath, "Info.xml")
            'If Not System.IO.File.Exists(DataPath & "\Info.xml") Then
            '    System.IO.File.Copy(ApplicationPath & "\AppData\Info.xml", DataPath & "\Info.xml")
            'Else
            '    System.IO.File.Copy(DataPath & "\Info.xml", ApplicationPath & "\AppData\Info.xml", True)
            'End If
            copyXmlFiles(OldPath, "Telnet.xml")
            'If Not System.IO.File.Exists(DataPath & "\Telnet.xml") Then
            '    System.IO.File.Copy(ApplicationPath & "\AppData\Telnet.xml", DataPath & "\Telnet.xml")
            'Else
            '    System.IO.File.Copy(DataPath & "\Telnet.xml", ApplicationPath & "\AppData\Telnet.xml", True)
            'End If
            copyXmlFiles(OldPath, "Station.xml")
            'If Not System.IO.File.Exists(DataPath & "\Station.xml") Then
            '    System.IO.File.Copy(ApplicationPath & "\AppData\Station.xml", DataPath & "\Station.xml")
            'Else
            '    System.IO.File.Copy(DataPath & "\Station.xml", ApplicationPath & "\AppData\Station.xml", True)
            'End If
            'If Not System.IO.File.Exists(DataPath & "\QRZKey.xml") Then
            '    System.IO.File.Copy(ApplicationPath & "\AppData\QRZKey.xml", DataPath & "\QRZKey.xml")
            'End If
            'copyXmlFiles(OldPath, "QRZKey.xml")
    ...
    Private Sub FillListview()
        ...
        'Dim history_list As New ArrayList
        'Dim new_item As Boolean

        'For Each hist As PJ In h
        '    new_item = True
        '    For Each item As PJ In nick_unique
        '        If hist.callsign = item.callsign.Split("/")(0) Then
        '            Exit For
        '            new_item = False
        '        End If

        '    Next
        '    If new_item Then history_list.Add(hist)
        'Next

        'For Each Item As PJ In history_list
        '    If Not nick_unique.Contains(Item) Then
        '        nick_unique.Add(Item)
        '    End If
    ...
    Private Function findCall3() As String
        Dim fspec() As String = _
        {"C:\Program Files\WSJT9\CALL3.TXT", _
         "C:\Program Files (x86)\WSJT9\CALL3.TXT", _
         "C:\Program Files\WSJT\CALL3.TXT", _
         "C:\Program Files (x86)\CALL3.TXT", _
         "C:\WSJT97\CALL3.TXT", _
         "C:\WSJT9\CALL3.TXT", _
         "C:\WSJT\CALL3.TXT"}
    ...
    Private Sub ListView1_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles StationsListView.MouseUp
        ...
        Select Case e.Button
            Case Windows.Forms.MouseButtons.Left
                ...
                'logBookFrm.Show()
                CallLabel.Text = SelectedStation.callsign.Split("/")(0)
                NameLabel.Text = SelectedStation.firstname
                GridLabel.Text = SelectedStation.locator
    ...
    Private Sub NewInfofromDialog(ByVal nickname As String, ByVal callsign As String, ByVal state As String, ByVal grid As String, ByVal firstname As String, _
                        ...
                        If Not item_found Then
                            ...
                            For Each item In logArray
                                If item.length > 1 Then
                                    If item(2).ToString = Mid(split_nick(0), 1, item(2).ToString.Length) Then
                                        npj.logged(-1) = item
                                        'npj.distance = MHDistance(myInfo.locator, npj.locator)
    ...
    Private Sub LoadColorsXMLDocument()
        ...
        Try
            ...
            Dim myColors = From c In xColors.<Pref>.<Callcolors>.<color> _
                    Select c
            Debug.Print("")
    ...
    Private Sub SaveColorsXMLDocument(ByVal myColors As ArrayList)
        Try
            xColors = <?xml version="1.0"?>
                      <Pref>
                          <Callcolors>
                              <%= From elem As Color In myColors _
        ...
        Try
            xInfo = <?xml version="1.0"?>
                    <Info>
                        ...
                        <webpageindex><%= WebPagesToolStripComboBox.SelectedIndex %></webpageindex>
                        ...
                        <ViewChat><%= Me.N5TMChatToolStripMenuItem1.Checked %></ViewChat>
                    </Info>
    ...
    Private Function ThisPJCInfo(ByVal lvi As ListViewItem) As PJ
        Dim pj As New PJ
        For Each item As PJ In nick_unique
            If item.callsign = lvi.SubItems(0).Text Then
                Return item
            End If
        Next
        Return Nothing
    ...
    Private Sub RunningButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RunningButton.Click
        If CallLabel.Text = "Callsign" Then
        ...
        Else
            ...
            If IsNumeric(Me.FreqTextBox.Text) Then
                ...
                If IsNumeric(QTFTextBox.Text) Then
                    az = QTFTextBox.Text + " deg"
                End If
                PostReponse(postMsg("~ Running ~ " + FreqTextBox.Text + " ~" + az + " " + ModeTextBox.Text + " 1st w/" + CallLabel.Text))
                RetrievePJPage.refreshtime = 5000
                QsoStartTime = Now
                Me.OneSecondTimer.Enabled = True
            Else
        ...
        Else
            ...
            If IsNumeric(Me.FreqTextBox.Text) Then
                ...
                If IsNumeric(QTFTextBox.Text) Then
                    az = QTFTextBox.Text + " deg"
                End If
                PostReponse(postMsg("~ Running ~ " + FreqTextBox.Text + " ~" + az + " " + ModeTextBox.Text + " 2nd w/" + CallLabel.Text))
                RetrievePJPage.refreshtime = 5000
                QsoStartTime = Now
                Me.OneSecondTimer.Enabled = True
            Else
    ...
    Private Sub N5TMChatToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles N5TMChatToolStripMenuItem.Click
    ...
    Private Sub MeteorScatterToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MeteorScatterToolStripMenuItem.Click
        Dim webAddress As String = "http://meteorscatter.org/"
        Process.Start(webAddress)
    End Sub

    Private Sub VirgoToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles VirgoToolStripMenuItem.Click
        Dim webAddress As String = "http://dl1dbc.net/Meteorscatter/"
        Process.Start(webAddress)
    End Sub
    ...
    Private Sub HB9QLoggerToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles HB9QLoggerToolStripMenuItem.Click
        Dim webAddress As String = "http://hb9q.ch/version2/index.php/loggedin-loggers"
        Process.Start(webAddress)
    End Sub
    ...
    Private Sub CallLabel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CallLabel.Click
        Clipboard.SetText(CallLabel.Text.Split("/")(0))
    End Sub

    Private Sub GridLabel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GridLabel.Click
        Clipboard.SetText(GridLabel.Text)
    End Sub

    Private Sub NameLabel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles NameLabel.Click
        Me.MsgTextBox.Text = CallLabel.Text.Split("/")(0) + " " + NameLabel.Text + ", "
    End Sub
    ...
    Private Sub UpdateCall3txtToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UpdateCall3txtToolStripMenuItem.Click
        ...
        End If
        Try
            Dim wFile As New System.IO.StreamWriter(Call3WSJTPath)

            ' first write header info
            For Each hdr In call3hdr
                wFile.WriteLine(String.Join(",", hdr))
            Next
            ...
            ' write trailer 
            For Each trlr As String() In call3trlr
                wFile.WriteLine(String.Join(",", trlr))
            Next
...
9:              Dim mFile As New System.IO.StreamWriter(Call3MAP65Path)

                ' first write header info
                For Each hdr In call3hdr
                    mFile.WriteLine(String.Join(",", hdr))
                Next
                ' now wright all elements
                ...
                ' write trailer 
                For Each trlr As Array In call3trlr
                    mFile.WriteLine(String.Join(",", trlr))
                Next
    ...
    Private Sub OnlineHelpDocumentToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OnlineHelpDocumentToolStripMenuItem.Click
        Dim webAddress As String = "http://meteorscatter.n5tm.com/chat/HSMS_ChatRoom/downloads/PJClient_User_Guide_version_1.7.3.x.pdf"
        Process.Start(webAddress)
    End Sub
